# Hint
该项目于 2026 年 6 月 22 日 起，正式停止维护。如果有PR我还是会合并一下的，issues不理了，让想维护的人消除issues（
如果有提PR的开发者，那你真的太勇敢了，居然会碰AI的屎山，也让我很感动，没想到还有人记着这个项目。
由于AI的屎山，加上主开发者，没错就是我，那狗吧，钱包空空，无法为AI付费。所以我将从0开始自学安卓开发，开发出一个新的云湖客户端，敬请不要期待，这个过程还是挺长滴~，后续我也想开发更多类型的安卓软件，来增强一下自己的编程能力。


# Experimental
在canary分支上进行更激进的修改

<p align="center">
  <img src="https://img.shields.io/badge/Kotlin-2.2.21-7F52FF?logo=kotlin" alt="Kotlin"/>
  <img src="https://img.shields.io/badge/Jetpack%20Compose-1.9.0-4285F4?logo=android" alt="Compose"/>
  <img src="https://img.shields.io/badge/Material%20You-3.0-757575?logo=material-design" alt="Material You"/>
  <img src="https://img.shields.io/badge/License-GPL%20v3-blue.svg" alt="License"/>
  <img src="https://img.shields.io/github/stars/Kauid323/Yhchat_md3?style=social" alt="Stars"/>
</p>

![image](https://github.com/user-attachments/assets/96b2dc98-0368-443c-bcb6-aa64f6eafe4f)


# 已经实现的功能：
- [x] 手机号/邮箱/用户Token登录
- [x] 删除/显示会话/管理置顶会话
- [x] 完整解析消息
- [x] Websocket消息实时同步/实时更新会话/草稿同步/流式消息
- [x] 解析和发送普通/HTML/Markdown/图片/文件/视频/语音/表情/指令/A2UI消息
- [x] 搜索用户/群聊/机器人，并且添加
- [x] 查看用户/机器人/群聊详情（并且实现protobuf解析）
- [x] 播放语音/下载文件消息，可保存语音到本地
- [x] 查看个人资料
- [x] 修改邀请码/名称/头像/查看云湖总用户
- [x] 修改密码/绑定邮箱
- [x] 搜索会话/口令进群
- [x] 创建群聊/机器人
- [x] 各种群聊管理功能...
- [x] 社区
  - 分区列表
  - 查看关注的分区
  - 查看我发布的文章/已收藏的文章/官方推荐文章
  - 分区文章列表
  - 解析Markdown语法文章
  - 发送普通(或Markdown)类型/评论/分享/点赞/收藏/打赏文章
  - 查看作者/评论者详情
  - 搜索文章/分区
  - 查看分区下绑定的群聊，并且支持申请加入
  - 删除/编辑自己的文章
  - 查看文章详情
  - 屏蔽/取消屏蔽发送文章的用户
  - 分区管理/查看管理分区的关注列表
  - 新建分区/管理分区/删除分区
- [x] 机器人
  - 查看自己创建的机器人列表
  - 编辑/删除机器人
  - 清空上下文
  - 设置机器人大模型相关参数
  - 连接机器人websocket接收消息测试
- [x] 金币
  - 查看金币余额
  - 金币兑换物品
  - 查看金币记录
  - 查看任务明细  
       
# Todo：
- 机器人变量显示
- 发送消息带机器人变量（有机器人变量时才会有）
- 修生成聊天记录功能
- 更强壮的html解析器
- 查找引用消息

> 最新构建请看[Actions](https://github.com/Kauid323/Yhchat_MD3/actions)，点击[Release](https://github.com/Kauid323/Yhchat_MD3/releases)下载，或[加入云湖群聊](https://yhfx.jwznb.com/share?key=csahOdGjPUYX&ts=1778391867)，使用高速下载通道

# 特别感谢
### [yhchatAPI](https://github.com/yh-Tpdev/yhchatAPI)
### [FlymeLive](https://github.com/wsu2059q/FlymeLive)
### [图标@星河间](https://github.com/Kauid323/Yhchat_MD3/issues/12)
### [HtmlText](https://github.com/ch4rl3x/HtmlText/)
### [Reorderable](https://github.com/Calvin-LL/Reorderable)
### [KsoupHtml](https://github.com/MohamedRejeb/Ksoup)
### [轻昼](http://github.com/shijuhao/QingZhouCE)

以及项目里用到的所有库

# 声明
此项目（Yhchat_MD3）是个人为了兴趣而开发, 仅用于学习和测试，请于下载后24小时内删除。 所用API皆从官方网站收集, 不提供任何破解内容。
