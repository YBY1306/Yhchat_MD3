    ...
    Box(
        ...
        if (layoutResult != null) {
            urlAnns.forEach { ann ->
                ...
                rects.forEach { r ->
                    Box(
                        modifier = linkBoxModifier(
                            annotatedString.text.substring(ann.start, ann.end),
                            ann.item
                        )
                            .offset(
                                x = with(density) { r.left.toDp().takeIf { it > 0.dp } ?: 1.dp },
                            ...
                            .clickable {
                                onUriClick?.invoke(ann.item) ?: uriHandler.openUri(ann.item)
                            }
                    )
                }
            }
